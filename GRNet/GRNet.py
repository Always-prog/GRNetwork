...
def vibor():
    ...
    if outputs[0][0] > outputs[1][0] and outputs[0][0] > outputs[2][0]:
    ...
    if outputs[1][0] > outputs[0][0] and outputs[1][0] > outputs[2][0]:
    ...
    if lose_igrok == False:
        if not(if_jump):
            if outputs[2][0] > outputs[0][0] and outputs[2][0] > outputs[1][0] and y_igrok > 5:
                if_jump = True
        else:
            ...
            if jump >= -10:
                ...
                jump = 10 #вызвращаю переменую отвечающую за прыжок на место
...
while pley == True:
    ...
    if keys[pygame.K_w]:
        outputs[2][0] = 0.9
######### НЕЙРОСЕТЬ ################
    inputs[0][0] = x_igrok / 1000
    inputs[1][0] = y_igrok / 1000
    inputs[2][0] = x_kol / 1000
